...
export default class Oauth2Scheme extends BaseScheme<typeof DEFAULTS> {
  ...
  public oidcVerifier: IdTokenVerifier
  ...
  constructor ($auth, options, ...defaults) {
    ...
    this.oidcVerifier = new IdTokenVerifier({
      issuer: options.issuer,
      jwksURI: options.endpoints.jwks,
  ...
  async checkJWT (token) {
    return new Promise((resolve, reject) => {
      this.oidcVerifier.verify(token, null, (error, payload) => {
        if (error) {
          return reject(error)
        }
        resolve(payload)
